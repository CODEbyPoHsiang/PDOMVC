...
require_once "controller/Music.php"; //控制器
require_once "model/MusicModel.php"; //Model 模型

$db = new Database();
$model = new MusicModel($db);
$controller = new Music($model);

if ($uri0 && $uri1 && $uri[0] === 'music' && $uri[1] === 'info') {         // Info//個人資料檢視
    $id = $_GET['id'];
    $controller->info($id);
    ...
    您要查找的頁面不存在</center></h2></body></html>';
}